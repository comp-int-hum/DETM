...
class DBDETMr(AbstractDETM):
    def __init__(
        ...
        super(DBDETMr, self).__init__(num_topics, word_list, embeddings)        
    ...
    def topic_embeddings(self, document_times):
        ...
        for t in range(1, self.num_windows):
            ...
            p_mu_t = alphas[t-1]
            logsigma_p_t = torch.log(self.delta * self.centroid_difference[t-1] * torch.ones(self.num_topics, self.embedding_size).to(self.device))
