...
class xDETMCoeff(AbstractDETM):
    ...
    def forward(self, document_word_counts, document_times):
        coeff = self.num_docs/document_word_counts.size(0)
        ...
        nelbo = reconstruction_loss.sum()*coeff + topic_embeddings_kld.sum() + document_topic_mixture_priors_kld.sum() + document_topic_mixtures_kld.sum()*coeff
        return (nelbo, reconstruction_loss*coeff, topic_embeddings_kld, document_topic_mixture_priors_kld, document_topic_mixtures_kld*coeff)
