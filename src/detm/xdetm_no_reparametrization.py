...
class xDETMnr(AbstractDETM):
    def __init__(
        ...
        super(xDETMnr, self).__init__(num_topics, word_list, embeddings)        
    ...
    def document_topic_mixture_priors(self, document_times):
        ...
        for t in range(1, self.num_windows):
            ...
            if self.training:
                ...
                p_mu_t = last_mu#etas[t-1]
                logsigma_p_t = torch.log(torch.exp(last_logsigma) + self.delta * torch.ones(self.num_topics,).to(self.device)) #torch.log(self.delta * torch.ones(self.num_topics, self.embedding_size).to(self.device))torch.log(self.delta * torch.ones(self.num_topics,).to(self.device))
