...
class DBETM(nn.Module):
    def __init__(
        ...
        self.num_windows = len(self.window_list)
        self.window_centroids = [(window_list[i][0] + window_list[i][0])/2 for i in range(self.num_windows)]
    ...
    def get_alpha(self): ## mean field
        ...
        for t in range(1, self.num_windows):
            ...
            p_mu_t = alphas[t-1]
            logsigma_p_t = torch.log(torch.exp(self.logsigma_q_alpha[:, t-1, :]) + self.delta * self.centroid_difference[t-1] * torch.ones(self.num_topics, self.rho_size).to(self.device))
    ...
    def get_eta(self, rnn_inp): ## structured amortized inference
        ...
        for t in range(1, self.num_windows):
            ...
            if self.training:
                ...
                p_mu_t = etas[t-1]
                logsigma_p_t = torch.log(torch.exp(self.logsigma_q_eta(inp_list[-2])) + self.delta * self.centroid_difference[t-1] * torch.ones(self.num_topics, self.rho_size).to(self.device))
